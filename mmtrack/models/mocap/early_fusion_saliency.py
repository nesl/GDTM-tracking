...
class EarlyFusion(BaseMocapModel):
    def __init__(self,
                 backbone_cfgs=None,
                 model_cfgs=None,
    ...
    def forward_track(self, datas, return_unscaled=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            import csv
            with open('/home/iobt/Desktop/attn.csv', 'w') as f:
                mywriter = csv.writer(f, delimiter=',')
                for layers in range(6):
                    for heads in range(8):
                        temp = torch.squeeze(attn_weights[layers])[heads,:]
                        fsize = [16,108,16,135,16,108,16,135,16,108,16,135]
                        start = 0
                        attn_mod = []
                        for ii in range(len(fsize)):
                            attn_mod.append(np.mean(temp.cpu().numpy()[start:start+fsize[ii]]))
                            start += fsize[ii]
                        mywriter.writerow(attn_mod) 
    ...
    def forward_train(self, datas, return_unscaled=False, **kwargs):
        ...
        for i in range(B):
            for j in range(T):
                ...
                dist = output['dist']
                means = dist.loc
                means[0][0][0].backward()
                saliency = all_embeds.grad.data[i,j].abs()
                import csv
                with open('/home/iobt/Desktop/saliency_map.csv', 'w') as f:
                    mywriter = csv.writer(f)
                    mywriter.writerows(saliency.cpu().numpy()) 
                # import matplotlib.pyplot as plt
                # fig, ax = plt.subplots(1, 2)
                # ax[0].imshow(all_embeds[i,j].cpu().detach().numpy())
                # ax[0].axis('off')
